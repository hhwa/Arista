        ...
        <td align="right" colspan="2">
        	<s:if test="#session.session_id !=null">
        		<s:if test="memParam.myteam == null">
        			...
        			<s:else>
					<input type="button" value="신청" onclick="aboutTeam('joinTeam.action?team_id=<s:property value="resultClass.team_id"/>','신청')"/>
					</s:else>
				</s:if>
				<s:else>
					<s:if test="teamInfoParam.team_admin == 1 || teamInfoParam.team_admin ==2">
						<input name="list" type="button" value="수정" class="inputb" onClick="aboutTeam('TeamModifyForm.action?team_no=<s:property value="team_no"/>&currentPage=<s:property value="currentPage"/>','수정')"/>
						<input name="list" type="button" value="삭제" class="inputb" onClick="aboutTeam('TeamDeleteForm.action?team_no=<s:property value="team_no"/>&currentPage=<s:property value="currentPage"/>','삭제')"/>
					</s:if>
				</s:else>
