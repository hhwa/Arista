...
public class MemberJoinAction extends ActionSupport
		implements Preparable, ModelDriven<memVO>, SessionAware, ServletRequestAware {
	// SQL 맵핑
...
//	private String fileUploadPath = System.getProperty("user.dir") + "\\upload\\";
	...
	private soloMatchVO soloMatchParam;
	...
	private List<joinSoloVO> myList = new ArrayList<joinSoloVO>();
	...
	public String nickcheck() throws Exception {
		...
		/*if(session.get("session_id")!=null) {
			String nick=(String) sqlMapper.queryForObject("memSQL.nickcheck2",(String)session.get("session_id"));
			if(nick.equals(memberResult.getM_nickname())) { //기존 닉네임과 같을경우
				nickcheckresult = 1;
	...
	public String deleteMem() throws Exception {
		...
		if(teamInfoParam.getTeam_admin()==2) {
			return ERROR;
		}
		...
		if(memberResult.getMyteam()!= null) {
			teamInfoParam = (teamInfoVO) sqlMapper.queryForObject("teamSQL.teamMember1", memberParam.getM_id());
			sqlMapper.delete("teamSQL.withdrawTeam",teamInfoParam);
		}
		
		myList = sqlMapper.queryForList("joinSoloSQL.mySoloMatchList",(String)session.get("session_id"));
		
		for(int i=0; i<myList.size(); i++) {
			List<joinSoloVO> listMap = myList;
			int match_no = listMap.get(i).getMatch_no();
			joinSoloParam.setMatch_no(match_no);
			joinSoloParam.setMem_id((String)session.get("session_id"));
			sqlMapper.delete("joinSoloSQL.cancelSoloMatch",joinSoloParam);
			
			int count = (int) sqlMapper.queryForObject("joinSoloSQL.joinSoloMatchCount", match_no);
			matchStateResult.setMatch_no(match_no);
			matchStateResult.setPeople_count(count);
			soloMatchParam.setGame_no(listMap.get(i).getGame_no());
	...
	public List<joinSoloVO> getMyList() {
		return myList;
	}

	public void setMyList(List<joinSoloVO> myList) {
		this.myList = myList;
	}
